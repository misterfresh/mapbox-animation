...
var programAttributeArgs = ['x', 'y', 'ox', 'oy', 'tx', 'ty', 'minzoom', 'maxzoom', 'labelminzoom', 'next_x', 'next_y', 'prev_x', 'prev_y'];
...
SymbolBucket.prototype.addSymbols = function(programName, quads, scale, keepUpright, alongLine, placementAngle) {
    ...
    for (var k = 0; k < quads.length; k++) {
        ...
        var index = addVertex(anchorPoint.x, anchorPoint.y, tl.x, tl.y, tex.x, tex.y, minZoom, maxZoom, placementZoom, nextPos.x, nextPos.y, prevPos.x, prevPos.y) - group.vertexStartIndex;
        addVertex(anchorPoint.x, anchorPoint.y, tr.x, tr.y, tex.x + tex.w, tex.y, minZoom, maxZoom, placementZoom, nextPos.x, nextPos.y, prevPos.x, prevPos.y);
        addVertex(anchorPoint.x, anchorPoint.y, bl.x, bl.y, tex.x, tex.y + tex.h, minZoom, maxZoom, placementZoom, nextPos.x, nextPos.y, prevPos.x, prevPos.y);
        addVertex(anchorPoint.x, anchorPoint.y, br.x, br.y, tex.x + tex.w, tex.y + tex.h, minZoom, maxZoom, placementZoom, nextPos.x, nextPos.y, prevPos.x, prevPos.y);
...
function SymbolInstance(anchor, line, shapedText, shapedIcon, layout, addToBuffers, index, collisionBoxArray, featureIndex, sourceLayerIndex, bucketIndex,
    ...
    this.nextX = nextPos.x;
    this.nextY = nextPos.y;
    this.prevX = prevPos.x;
    this.prevY = prevPos.y;

    if (this.hasText) {
        this.glyphQuads = addToBuffers ? getGlyphQuads(anchor, shapedText, textBoxScale, line, layout, textAlongLine, nextPos, prevPos) : [];
    ...
    if (this.hasIcon) {
        this.iconQuads = addToBuffers ? getIconQuads(anchor, shapedIcon, iconBoxScale, line, layout, iconAlongLine, nextPos, prevPos) : [];
