...
vec2 beziersTransition(mediump float completion, vec2 pos, vec2 next_pos, vec2 prev_pos);
...
void main() {
    ...
    if (u_transition_style == 2) current_pos = previous_defined ? beziersTransition(u_completion, a_pos, a_next_pos, a_prev_pos) : a_pos;
